...
public class SenatorialZoneExcelReadUtil {
	
	private static Cell cell;
	private static Map<String, List<SenatorialDistrict>> SenDistricts = new HashMap<String, List<SenatorialDistrict>>();
	private static Map<String, Object> rowData;
	private static String stateName = "";

	    public static Map<String, List<SenatorialDistrict>> readSheet(byte[] bytes) throws Exception {
	    	List<SenatorialDistrict> zones=null;
	        ...
	        try (InputStream file = new ByteArrayInputStream(bytes)) {
	            ...
	            while (rowIterator.hasNext()) {
	                ...
	                while (cellIterator.hasNext()) {
	                    ...
	                    columnCount++;
	                      System.out.println("point "+rowCount+" : "+columnCount+" --- "+getValue().toString());
	                      if( getStates().contains(getValue().toString())){
	                    	  rowCount=1;
	                    	  stateName=getValue().toString();
	                    	  zones = new ArrayList<SenatorialDistrict>();
	                    	   SenDistricts.put(stateName,zones);
	                    	  System.out.println(">>>>>>>>>>>>>>>>>>>>>1 : "+SenDistricts.get(stateName));
                			  
	                      }else{
	                    	  if (rowCount >=3 && columnCount ==2) {
	                    		 	  System.out.println(">>>>>>>>>>>>>>>>>>>>>2 : "+SenDistricts.get(stateName));
	                    			  SenDistricts.get(stateName).add(new SenatorialDistrict(getValue().toString()));
...
//	        out.close();
	...
	public static void main(String[] args) {
		try {
		    File file = new File("C:\\Java projects\\truevote\\Name-of-Senatorial-DistrictsFederal-and-State-Const.xls");
		    byte[] byteArray = new byte[(int) file.length()];
		    byteArray = FileUtils.readFileToByteArray(file); 
		    SenatorialZoneExcelReadUtil.readSheet(byteArray);
		 }catch(Exception e){
		     e.printStackTrace();
